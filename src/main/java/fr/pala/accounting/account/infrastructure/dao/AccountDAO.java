...
public class AccountDAO {
    ...
    public Account addAccount(String email, Account account) throws InvalidFieldException {
        ...
        List<AccountMongoModel> accountMongoModels = AccountAdapter.transactionListToModelList(accounts);
        Query query = new Query();
        query.addCriteria(Criteria.where("_id").is(user.getUser_id()));
        Update update = new Update();
        update.set("accounts", accountMongoModels);
    ...
    public Account updateAccount(String email, Account account) throws InvalidFieldException, AccountNotUpdatedException {
        AccountMongoModel accountMongoModel = AccountAdapter.accountToModel(account);
        UserModel user = userDAO.getUserByEmail(email);
        List<AccountMongoModel> accounts = user.getAccounts();

        accounts.replaceAll(acc -> acc.getId().equals(accountMongoModel.getId()) ? accountMongoModel : acc);
        ...
        if (mongoTemplate.findAndModify(query, update, UserModel.class) != null) {
            return AccountAdapter.modelToAccount(accountMongoModel);
        }
